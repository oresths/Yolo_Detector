...
void delta_region_class(float *output, float *delta, int index, int classn, int classes, tree *hier, float scale, float *avg_cat)
{
    int i, n;
    if(hier){
        float pred = 1;
        while(classn >= 0){
            pred *= output[index + classn];
            int g = hier->group[classn];
            int offset = hier->group_offset[g];
            for(i = 0; i < hier->group_size[g]; ++i){
                delta[index + offset + i] = scale * (0 - output[index + offset + i]);
            }
            delta[index + classn] = scale * (1 - output[index + classn]);

            classn = hier->parent[classn];
        }
        *avg_cat += pred;
    } else {
        for(n = 0; n < classes; ++n){
            delta[index + n] = scale * (((n == classn)?1 : 0) - output[index + n]);
            if(n == classn) *avg_cat += output[index + n];
        }
    }
}
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int classn = state.truth[t*5 + b*l.truths + 4];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        int index = size*n + b*l.outputs + 5;
                        float p = get_hierarchy_probability(l.output + index, l.softmax_tree, classn);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, index, classn, l.classes, l.softmax_tree, l.class_scale, &avg_cat);
                    ++class_count;
                    onlyclass = 1;
                    break;
                }
        ...
        for(t = 0; t < 30; ++t){
            ...
            int classn = state.truth[t*5 + b*l.truths + 4];
            if (l.map) classn = l.map[classn];
            delta_region_class(l.output, l.delta, best_index + 5, classn, l.classes, l.softmax_tree, l.class_scale, &avg_cat);
            ++count;
            ++class_count;
        }
    }
